...
from .iblt import IBLT, IBLTConfig, estimate_iblt_size, HASH_SIZE
...
class CosignedSyncState:
    ...
    def is_valid(self, now_ms: Optional[int] = None) -> Tuple[bool, str]:
        """
...
class SyncSession:
    ...
    peer_state: Optional[SyncState] = None
    common_size: int = 0
...
class SyncResult:
    ...
    fork_proof: Optional[ForkProof] = None
...
class NonceCache:
    ...
    async def check_and_add(self, nonce: bytes, timestamp_ms: int) -> bool:
        ...
        try:
            async with asyncio.timeout(NONCE_CHECK_TIMEOUT):
            ...
            raise RuntimeError("Nonce check timed out")
...
class WitnessCoordinator:
    ...
    def sign_state(self, state: SyncState, session_nonce: bytes) -> Optional[WitnessSignature]:
        """
...
class FrontierSync:
    ...
    def __init__(
        ...
        witness_coordinator: Optional[WitnessCoordinator] = None,
        goal_id: str = "default",
        iblt_config: Optional[IBLTConfig] = None,
    ):
        self._mmr = mmr
            ...
            if only_peer:
                ...
                for entry in missing_entries:
                    ...
                    if entry_hash in only_peer:
                        ...
                        logger.warning(f"Received entry with unexpected hash")
