...
- `run_idi_trainer.py`: CLI entrypoint (`python run_idi_trainer.py --out build/idi_traces`) that emits Tau-ready `*.in` files plus a manifest JSON.
...
Output directories can be copied directly into a Tau spec folder (e.g., `idi/specs/V38_Minimal_Core/inputs/`) before running the Tau binary.
