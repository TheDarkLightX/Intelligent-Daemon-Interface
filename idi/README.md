...
| `specs/` | Tau specifications (`V38_Minimal_Core`, `Q_Layered_Strategy`, and the `idi_core` stream contract). |
...
2. Use `python -m idi.devkit.builder --config ... --out ... --install-inputs specs/V38_Minimal_Core/inputs` (or `python -m idi.devkit.build_layers`) to produce streams + manifests; the batch plan also hydrates `specs/Q_Layered_Strategy/inputs` with price/weight echoes for the layered spec.
