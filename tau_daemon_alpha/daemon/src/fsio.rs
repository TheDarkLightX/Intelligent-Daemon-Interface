...
impl TauRunner {
    pub fn new(tau_bin: PathBuf, inputs_dir: PathBuf, outputs_dir: PathBuf, timeout_ms: u64) -> Self {
        ...
        Self { tau_bin: resolved_tau_bin, inputs_dir, outputs_dir, timeout_ms }
    }
    ...
    pub async fn run_spec(&self, spec_path: &Path) -> Result<()> {
        ...
        #[cfg(target_family = "unix")]
        {
            ...
            if !link_inputs.exists() {
                if let Err(e) = unix_fs::symlink(&self.inputs_dir, &link_inputs) {
                    error!("Failed to create inputs symlink {:?} -> {:?}: {}", link_inputs, self.inputs_dir, e);
                }
            }
            ...
            if !link_outputs.exists() {
                if let Err(e) = unix_fs::symlink(&self.outputs_dir, &link_outputs) {
                    error!("Failed to create outputs symlink {:?} -> {:?}: {}", link_outputs, self.outputs_dir, e);
                }
            }
        }
        ...
        .await {
            ...
            Err(_) => {
                error!("Tau spec timed out after {}ms", self.timeout_ms.saturating_mul(2));
    ...
    pub async fn run_monitors(&self, specs_root: &Path) -> Result<()> {
        ...
        let cooldown_monitor_path = specs_root.join("monitors_auditors/06_rate_limiter_cooldown.tau");
