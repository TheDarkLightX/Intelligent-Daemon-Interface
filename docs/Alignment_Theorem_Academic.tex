...
\midrule
$S(t)$ & Circulating AGRS supply at tick $t$ & Lean model definitions \\
$r(t)$ & Deflation rate bounded in $[0.01,0.50]$ & Agent V54 specification \\
$M(t)$ & Scarcity multiplier $S_0/S(t)$ & Lemma \texttt{scarcity\_limit\_infinity} \\
$E(t)$ & Network-wide EETF value in $[0,3]$ & Ethics aggregator library \\
$e$ & Account-level EETF score & Ethics aggregator library \\
$B$ & Balance driving rewards & Agent V54 state machine \\
$X$ & Position size / exposure & Agent V54 risk budget \\
$\mathrm{tier}(e)$ & Multiplier $\{1,3,5\}$ keyed to $e$ & Dynamic Base Reward module \\
$\tau_k$ & Target tier multiplier & Derived from $\mathrm{tier}(e)$ \\
$P(t)$ & Economic pressure $M(t)E(t)$ & Derived definition in text \\
\bottomrule
\end{tabular}}
\caption{Key assumptions shared by the Lean proof and Tau specifications (file references: proofs/AlignmentTheorem.lean, specification/agent4\_testnet\_v54.tau, and the ethics libraries).}
...
\begin{enumerate}[leftmargin=*]
    \item The Lean development (\emph{proofs/AlignmentTheorem.lean}) introduces lemmas \texttt{supply\_limit\_zero} and \texttt{scarcity\_limit\_infinity}, proving $S(t)\to 0$ and $M(t)\to\infty$ under bounded deflation.
    \item Lemma \texttt{alignment\_invariant\_holds} shows that at sufficiently high pressure $P(t)$, any positive reward implies $e\ge 1$.
    \item The EV limits \texttt{ethical\_ev\_limit\_pos\_infinity} and \texttt{unethical\_ev\_limit\_neg\_infinity} produce a constructive threshold $S^\star$ with $\mathrm{EV}(e_{\text{ethical}}, t) > \mathrm{EV}(e_{\text{opp}}, t)$ for all $M(t) > S^\star$, matching the Tau EV function.
\end{enumerate}
\end{proof}
...
We complement the formal stack with stochastic simulations and symbolic game-theory tooling. The script \emph{analysis/simulations/run\_alignment\_simulations.py} sweeps scarcity growth rates, stochastic EETF shocks, and adversarial injections, producing the dataset in \emph{analysis/simulations/alignment\_sim\_results.csv}. For each scenario we log convergence steps, final ethical share, and tunable parameters $(\text{growth}, \epsilon, g_{\max})$. In parallel, \emph{verification/tau\_exact\_simulator.py} reproduces Tau execution traces under the same stimuli so that Lean invariants, Tau outputs, and Python simulations all observe the identical transition ordering. The payoff matrices and replicator gradients described later inherit their coefficients directly from these runs as well as the EV constants embedded in \emph{specification/agent4\_testnet\_v54.tau}.

\section{Results}
\subsection{Trace Coverage and Runtime Metrics}
Table~\ref{tab:fsm-coverage} summarizes the FSM metrics reported in docs/VERIFICATION\_SUMMARY.md. All library FSMs achieve full state and transition coverage, while the core agent maintains $10/10$ reachable states with $223/320$ transitions exercised automatically. The Tau CLI runs captured in outputs/tau\_runs/summary.json are shown in Table~\ref{tab:tau-runs}; the \texttt{exit\_139} status reflects the Alpha interpreter's limitation on historical stream replay, motivating the mirrored-input harness described earlier.
...
\bottomrule
\end{tabular}
\caption{FSM coverage statistics aggregated from docs/VERIFICATION\_SUMMARY.md. Core-agent transition coverage currently sits at $223/320$ (69.7\%) with remaining transitions exercised via targeted SMT queries.}
...
\bottomrule
\end{tabular}
\caption{Simulation summary extracted from analysis/simulations/alignment\_sim\_results.csv. Growth denotes scarcity drift, $\epsilon$ controls stochasticity, and $g_{\max}$ bounds adversarial injections.}
...
\end{tikzpicture}
\caption{Representative payoff matrix parameterized by the EV configuration in specification/agent4\_testnet\_v54.tau. Ethical strategies dominate once scarcity exceeds the Lean-derived threshold.}
...
The threat model document under \emph{docs} tracks adversarial pressure across four fronts: the aggregation layer $\mathcal{A}$, pointwise revision, extralogical primitives, and economic stressors. The current design mitigates Sybil or credential spam via stake-weighted throttling and commits to trimmed-mean fallbacks when stake concentration spikes. Pointwise revision safety is enforced by replaying only the targeted formulas and by guarding constitutional predicates in Lean, while extralogical hooks (commit-reveal, oracle monitors) inherit regression tests from the MEV/Oracle safety library. Finally, simulation campaigns inject tail-risk shocks (drop-in $E(t)$, liquidity freezes) so that scarcity-driven burns remain bounded even under malicious order flow.

\subsection{Limitations and Future Work}
Three limitations remain before journal submission. (i) Preference aggregation presently assumes truthful stake-weighted submissions; future work includes zk-attested identities and formal bounds on adversarial weight $\lambda$ so that coalitions cannot steer $E(t)$ by more than $\delta$. (ii) Transition coverage is $69.7\%$ for the core agent because some timed transitions require bespoke stimuli; extending the SMT harness in \emph{verification/verify\_transitions\_tau.py} to cover the remaining 97 transitions is ongoing. (iii) The Tau Alpha binary still aborts runs involving historical streams and $bv[256]$, so we rely on the Python exact simulator; reproducing the same traces with a production daemon (see the \emph{tau\_daemon\_alpha} workspace) is part of the deployment roadmap.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Stress testing}: the simulation suite and tau-testnet traces should cover tail-risk scenarios; analysis/simulations/run\_alignment\_simulations.py and docs/SIMULATION\_RESULTS.md are first steps.
\end{itemize}

\section{Data and Code Availability}
\begin{itemize}[leftmargin=*]
    \item Repository bundle: the \emph{alignment theorem package} directory mirrors the GitHub upload layout.
    \item Formal proofs: run \texttt{lake build} inside the \emph{proofs} folder.
    \item Tau specs: compile via the \emph{scripts/run\_tau\_specs.py} helper.
    \item Trace harness: the \emph{verification} folder includes \texttt{tau\_exact\_simulator.py} plus SMT utilities.
    \item Simulations: regenerate with \emph{analysis/simulations/run\_alignment\_simulations.py}.
    \item Packaging instructions: see the README shipped inside the alignment theorem package.
    \item Mirrored inputs/outputs: the \emph{outputs/tau\_runs} folder lets reviewers compare traces without rebuilding the Tau Alpha binary (current limit $bv[32]$).
\end{itemize}
\section{Conclusion}
...
We are grateful to Ohad Asor and the Tau core developers for building and maintaining the language, daemon, and solver infrastructure that made this work possible, and to the Tau community contributors who continually refine the ethical models and test harnesses used throughout the Alignment Theorem program.
