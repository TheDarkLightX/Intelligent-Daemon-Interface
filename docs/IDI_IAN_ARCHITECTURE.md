...
### Reusable FSM motifs
- **V35/V38 kernels** (see `specification/agent4_testnet_v35.tau` and `specification/V38_Minimal_Core/agent4_testnet_v38.tau`): provide the canonical 2-bit timer, nonce gating, and burn tracker used by all downstream specs.
- **Demo suite** (`AlignmentTheorem/demos/intelligent_agent`): showcases 4-state FSM entry/exit logic with mirrored inputs; we reuse its smart-entry timer as the base for `idi_core`.
...
3. **Daemon adapter** verifies the receipt, mirrors actions into `specification/V38_Minimal_Core/agent4_testnet_v38.tau` inputs, and logs proofs in the ledger. Tau never sees the table, only the index + proof flag; we keep optional `q_*_echo` outputs to audit what was consumed.
...
- **Mirrors/guards:** `specification/libraries/idi_core/idi_core.tau` standardizes stream names, mirror outputs, and de-bounce timers so each spec can read the same schema with minimal cyclomatic complexity.

## 5. Training libraries (Python + Rust)
- Python package under `analysis/q_tables/python/idi_iann/` exposes:
...
- Rust crate under `analysis/q_tables/rust/idi_iann/` mirrors the same abstractions (traits for env, policy, serializer) and ships unit/property tests so both stacks stay aligned.
...
| Path | Purpose |
|------|---------|
| `analysis/q_tables/python/idi_iann/` | Python training toolkit + CLI + tests. |
| `analysis/q_tables/rust/idi_iann/` | Rust mirror crate for deterministic cross-checks + Clippy gate. |
| `specification/libraries/idi_core/` | Stream contract + helper predicates for specs. |
| `docs/IDI_IAN_ARCHITECTURE.md` | This blueprint (kept in sync as architecture evolves). |
| `docs/IDI_zk_privacy.md` | Proof stack / licensing reference. |
| `AlignmentTheorem/demos/intelligent_agent/idi_demo/` | End-to-end replay harness with curated inputs + AoT verification. |
...
1. Generate fresh lookup artifacts: `analysis/q_tables/python/run_idi_trainer.py --out build/idi_traces`.
2. Feed artifacts into zk coprocessor; archive `(manifest, proof)` bundle.
3. Drop `.in` files + proof receipts into spec folder (`specification/V38_Minimal_Core/inputs/`).
