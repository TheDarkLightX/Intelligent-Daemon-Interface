"""Experimental spec generator v2 using stream contracts.

This keeps the legacy spec_generator.py intact while allowing contract-driven
input/mirror declarations for bundles that want tighter coupling with stream
...
class TauSpecGeneratorV2:
    """Generates Tau-language specs using the stream contract registry."""
    ...
    def generate_v38_spec(
        ...
        *,
        ...
        """Generate V38 spec from contract definitions."""
        contract = get_contract(contract_name)
        inputs = [s for s in contract if s.role == "input"]

        spec_lines = [
            "# IDI-ready agent spec (V38 Minimal Core) - contract-driven",
        ...
        spec_lines.append("")
        spec_lines.append("# === INPUT MIRRORS (Interpreter requirement) ===")
        for idx, stream in enumerate(inputs):
            spec_lines.append(
                f"o_i{idx}:{stream.tau_type} = out file(\"outputs/i{idx}_mirror.out\")."
            )
        ...
        mirror_bindings = [f"(o_i{idx}[t] = i{idx}[t])" for idx in range(len(inputs))]
        if mirror_bindings:
            spec_lines.append(" &&")
    ...
    def generate_layered_spec(
        ...
        *,
        ...
        """Generate layered strategy agent spec (same as v1)."""
        spec_lines = [
...
def generate_spec_from_config_v2(
    config_path: Path,
    output_path: Path,
    spec_type: str = "v38",
    contract_name: str = "v38",
) -> None:
    """Generate Tau spec from config file using the v2 generator."""
    import json
    from idi.training.python.idi_iann.config import TrainingConfig

    config_dict = json.loads(config_path.read_text())
    config = TrainingConfig(**config_dict)
    generator = TauSpecGeneratorV2(config)
